...
@interface ZFCandleChart()<ZFGenericAxisDelegate>
...
- (void)drawLine{
    id subObject = self.circleArray.firstObject;
    if ([subObject isKindOfClass:[ZFCircle class]]) {
        [self.genericAxis.layer addSublayer:[self lineShapeLayer:self.circleArray andWithMinValue:self.genericAxis.xMinValueArray andWithMaxValue:self.genericAxis.xMaxValueArray index:0]];
    }else if ([subObject isKindOfClass:[NSArray class]]){
        for (NSInteger i = 0; i < self.circleArray.count; i++) {
            [self.genericAxis.layer addSublayer:[self lineShapeLayer:(NSMutableArray *)self.circleArray[i] andWithMinValue:self.genericAxis.xMinValueArray[i] andWithMaxValue:self.genericAxis.xMaxValueArray[i] index:i]];
        }
    }
}
 ...
 *  @return CAShapeLayer
 */
- (CAShapeLayer *)lineShapeLayer:(NSMutableArray *)array andWithMinValue:(NSMutableArray *)minArray andWithMaxValue:(NSMutableArray *)maxArray index:(NSInteger)index{
    NSMutableArray * valuePointArray = [self cachedValuePointArray:array andWithMinValue:minArray andWithMaxValue:maxArray];
    
    ZFCandle * layer = [ZFCandle lineWithValuePointArray:valuePointArray isAnimated:self.isAnimated shadowColor:self.shadowColor padding:self.genericAxis.groupPadding];
...
- (void)circleAction:(ZFCircle *)sender{
    if ([self.delegate respondsToSelector:@selector(candleChart:didSelectCircleAtCandleIndex:circleIndex:circle:popoverLabel:)]) {
        ...
        [self.delegate candleChart:self didSelectCircleAtCandleIndex:sender.lineIndex circleIndex:sender.circleIndex circle:sender popoverLabel:popoverLabel];
...
- (void)popoverAction:(ZFPopoverLabel *)sender{
    if ([self.delegate respondsToSelector:@selector(candleChart:didSelectPopoverLabelAtCandleIndex:circleIndex:popoverLabel:)]) {
        [self.delegate candleChart:self didSelectPopoverLabelAtCandleIndex:sender.groupIndex circleIndex:sender.labelIndex popoverLabel:sender];
...
- (void)removeAllSubLayer{
    ...
    for (CALayer * layer in subLayers) {
        if ([layer isKindOfClass:[ZFCandle class]]) {
...
- (void)strokePath{
    if ([self.dataSource respondsToSelector:@selector(valueArrayInGenericChart:)]) {
        ...
        self.genericAxis.xMinValueArray = self.genericAxis.xMaxValueArray = self.genericAxis.xLineValueArray;
    }
    
    if ([self.dataSource respondsToSelector:@selector(minValueArrayInCandleChart:)]) {
        self.genericAxis.xMinValueArray = [NSMutableArray arrayWithArray:[self.dataSource minValueArrayInCandleChart:self]];
    }
    
    if ([self.dataSource respondsToSelector:@selector(maxValueArrayInCandleChart:)]) {
        self.genericAxis.xMaxValueArray = [NSMutableArray arrayWithArray:[self.dataSource maxValueArrayInCandleChart:self]];
    }
    ...
    if (self.isResetAxisLineMaxValue) {
        ...
        self.genericAxis.yLineMaxValue = [[ZFMethod shareInstance] cachedMaxValue:self.genericAxis.xMaxValueArray];
    ...
    if (self.isResetAxisLineMinValue) {
        if ([self.dataSource respondsToSelector:@selector(axisLineMinValueInGenericChart:)]) {
            if ([self.dataSource axisLineMinValueInGenericChart:self] > [[ZFMethod shareInstance] cachedMinValue:self.genericAxis.xMinValueArray]) {
                self.genericAxis.yLineMinValue = [[ZFMethod shareInstance] cachedMinValue:self.genericAxis.xMinValueArray];
    ...
    if ([self.delegate respondsToSelector:@selector(groupWidthInCandleChart:)]) {
        self.genericAxis.groupWidth = [self.delegate groupWidthInCandleChart:self];
    }
    
    if ([self.delegate respondsToSelector:@selector(paddingForGroupsInCandleChart:)]) {
        self.genericAxis.groupPadding = [self.delegate paddingForGroupsInCandleChart:self];
    }
    
    if ([self.delegate respondsToSelector:@selector(circleRadiusInCandleChart:)]) {
        _radius = [self.delegate circleRadiusInCandleChart:self];
    }else{
        _radius = ZFLineChartCircleRadius;
    }
    
    if ([self.delegate respondsToSelector:@selector(candleWidthInCandleChart:)]) {
        _lineWidth = [self.delegate candleWidthInCandleChart:self];
    }
    
    if ([self.delegate respondsToSelector:@selector(valuePositionInCandleChart:)]) {
        self.valuePositionArray = [NSMutableArray arrayWithArray:[self.delegate valuePositionInCandleChart:self]];
    }else{
 ...
 *  计算点的位置
 */
- (NSMutableArray *)cachedValuePointArray:(NSMutableArray *)circleArray andWithMinValue:(NSMutableArray *)minArray andWithMaxValue:(NSMutableArray *)maxArray {
    NSMutableArray * valuePointArray = [NSMutableArray array];
    for (NSInteger i = 0; i < circleArray.count; i++) {
        CGFloat maxPercent = ([self.genericAxis.xMaxValueArray[i] floatValue] - self.genericAxis.yLineMinValue) / (self.genericAxis.yLineMaxValue - self.genericAxis.yLineMinValue);
        if (maxPercent > 1) {
            maxPercent = 1.f;
        }
        
        CGFloat maxHeight = self.genericAxis.yLineMaxValueHeight * maxPercent;
        CGFloat max_yPos = self.genericAxis.axisStartYPos - maxHeight;
        
        CGFloat minPercent = ([self.genericAxis.xMinValueArray[i] floatValue] - self.genericAxis.yLineMinValue) / (self.genericAxis.yLineMaxValue - self.genericAxis.yLineMinValue);
        
        CGFloat minHeight = self.genericAxis.yLineMaxValueHeight * minPercent;
        CGFloat min_yPos = self.genericAxis.axisStartYPos - minHeight;
        ...
        NSDictionary * dict = @{ZFLineChartXPos:@(circle.center.x), ZFLineChartYPos:@(circle.center.y), ZFLineChartMinYPos:@(min_yPos), ZFLineChartMaxYPos:@(max_yPos), ZFLineChartIsHeightEqualZero:@(isHeightEqualZero)};
